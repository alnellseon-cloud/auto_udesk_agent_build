...
**Codex 就是智能层。** 本项目不需要任何 AI API Key。
用户用自然语言在对话框里描述需求 → Codex 理解并设计工作流 → 直接调用 UAgent 平台 API 完成搭建/修改/测试。
...
```
之后运行脚本使用 `.venv/bin/python xxx.py`，或先执行 `source .venv/bin/activate` 再使用 `python xxx.py`。
...
| 文件 | 更新内容 | 读者 |
|---|---|---|
| `WORKFLOW.md` | 节点状态、已知问题、待办事项（保持当前快照准确） | Codex（下次会话自动读取） |
...
- Codex 记忆根目录是 `~/.codex/memories/`。只有用户明确要求更新记忆时，才在 `~/.codex/memories/extensions/ad_hoc/notes/` 新增一份 `<timestamp>-<short-slug>.md` 更新说明；不要直接修改 `MEMORY.md`、`rollout_summaries/` 或既有记忆文件。
- 旧版 Claude Code 项目记忆可能位于 `~/.claude/projects/<project-key>/memory/`，仅作为只读迁移来源。
...
编排过程中若发现以下情况，主动提议更新 AGENTS.md，说明原因，等用户确认后再写入：
- 发现新的节点类型或节点参数
- 发现平台接口行为与现有文档描述不符
- 某类场景形成了可复用的固定模板
